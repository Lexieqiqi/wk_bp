...
def generate(args_list):
    ...
    # 配置模拟退火优化器的参数，并实例化优化器对象。
    optim_config = {
        'switch_possibility': args.switch_possibility, # 决定是否切换状态的可能性
        'starting_temperature': args.starting_temperature, # 起始温度，影响算法探索的初始状态
        'temperature_decay': args.temperature_decay,  # 温度衰减率，每次迭代后温度逐步降低
        'annealing_period': args.annealing_period, # 每次温度更新的周期
        'step_size': args.step_size, # 步长，决定每次状态变化的幅度
        'stepsize_period': args.stepsize_period, # 步长更新的周期
        'mu': args.mu, # 退火过程中的参数，用于调整算法行为
        'device': device # 计算设备（如GPU）
    }
    optimizer = Annealing(hand_model, **optim_config)

    # optimize
    # 计算初始能量，包括不同的能量组成部分（如位移能量、穿透能量等）
    weight_dict = dict(
        w_dis=args.w_dis,
        w_pen=args.w_pen,
        w_spen=args.w_spen,
    ...
    # 计算梯度用于后续优化
    # cal_energy函数计算当前手部姿态的总能量及其组成部分（例如位移能量、穿透能量等）。
    energy, E_fc, E_dis, E_pen, E_spen, E_joints = cal_energy(hand_model, object_model, verbose=True, **weight_dict)

    # 并使用反向传播计算能量的梯度，为后续优化步骤提供方向。
    energy.sum().backward(retain_graph=True)

    # 在每一步中，尝试进行一步优化，生成新的状态。
    # 计算新的能量值及其梯度。
    # 根据模拟退火算法决定是否接受新状态，并更新能量值。
    for step in range(1, args.n_iter + 1):
        ...
        with torch.no_grad():
            # 决定是否接受新的状态，并返回一个布尔数组，表示哪些新状态被接受
            accept, t = optimizer.accept_step(energy, new_energy)
            # 函数内部实现了模拟退火算法的接受准则
            # 如果新状态的能量更低，则总是接受新状态
            # 如果新状态的能量更高，则以一定概率接受新状态
    ...
    # save results
    # 构建抓取姿势的数据字典，包括位姿（位置和姿态）、能量信息等
    # 将抓取姿势结果保存为.npy文件
    translation_names = ['WRJTx', 'WRJTy', 'WRJTz'] # 手部末端位姿平移分量的名称
    rot_names = ['WRJRx', 'WRJRy', 'WRJRz'] # 手部末端位姿的旋转分量的名称
    joint_names = [
